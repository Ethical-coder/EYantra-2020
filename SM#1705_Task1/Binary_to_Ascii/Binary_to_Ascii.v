...
always @ (posedge clk)
	begin
		 ...
		 else if(i_int<12 & i_int>=0)                 //shift until all the binary bits have been shifted, which happens when i_int reaches 13
			begin
				/*
					Shift ADD-3 Algorithm
				*/
				r_shift_int={r_shift_int[14:0],r_int_part[11-i_int]};
				if(r_shift_int[3:0]>4 && i_int<11)begin
					r_shift_int[3:0]=r_shift_int[3:0]+3;
				end
				if(r_shift_int[7:4]>4 && i_int<11)begin
					r_shift_int[7:4]=r_shift_int[7:4]+3;
				end
				if(r_shift_int[11:8]>4 && i_int<11)begin
						r_shift_int[11:8]=r_shift_int[11:8]+3;
				end
				if(r_shift_int[15:12]>4 && i_int<11)begin
...
////////// Convert binary to bcd by using the Shift Add-3 algorithm. The below block is for Interger part of the data /////////////
	...
	begin
		...
		*/ 
		...
		if(i_dec==0 && r_dec_part!=dec_part)begin
			r_dec_part<=dec_part;
		end
		else if(i_dec<8 &&i_dec>=0)begin
			r_shift_dec={r_shift_dec[6:0],dec_part[7-i_dec]};
			if(r_shift_dec[3:0]>4 && i_dec<7)begin
				r_shift_dec[3:0]=r_shift_dec[3:0]+3;
			end
			if(r_shift_dec[7:4]>4 && i_dec<7)begin
...
always@*
	begin
		if (conversion_complete_int)				// Monitor if BCD conversion of integer part is done or not, if done then prceed with ascii conversion
		begin
		/* 
		WRITE YOUR CODE HERE TO CONVERT BCD TO ASCII (Integer part)
		*/
		r_int_ascii={4'b0011,r_bcd_int[15:12],4'b0011,r_bcd_int[11:8],4'b0011,r_bcd_int[7:4],4'b0011,r_bcd_int[3:0]};
		end
	end
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
...
/////////////////// The Below always block is same as above. This block is for the decimal part //////////////////////
	always @*
	begin
		if (conversion_complete_dec)				// Monitor if BCD conversion of decimal part is done or not, if done then prceed with ascii conversion.
		begin
		/* 
		WRITE YOUR CODE HERE TO CONVERT BCD TO ASCII (Decimal part)
		*/
			r_dec_ascii={4'b0011,r_bcd_dec[7:4],4'b0011,r_bcd_dec[3:0]};
		end
	end
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
endmodule 